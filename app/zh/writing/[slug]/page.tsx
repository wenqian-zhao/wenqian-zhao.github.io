...
export async function generateMetadata({ params }: Props): Promise<Metadata> { const writing = findWriting((await params).slug); if (!writing) return { title: "文章不存在 — 赵文茜" }; const zh = chineseWritingMeta[writing.slug]; const title = `${writing.title} — 赵文茜`; return { title, description: zh.description, openGraph: { title, description: zh.description, type: "article", images: [] }, twitter: { card: "summary", title, description: zh.description, images: [] } }; }
